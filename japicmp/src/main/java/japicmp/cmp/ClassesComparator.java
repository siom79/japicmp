    ...
    private void sortIntoLists(Map<String, CtClass> oldClassesMap, Map<String, CtClass> newClassesMap) {
        ...
        for(CtClass newCtClass : newClassesMap.values()) {
            ...
            if(oldCtClass == null) {
                ...
                JApiClass jApiClass = new JApiClass(this.jarArchiveComparator, newCtClass.getName(), Optional.<CtClass>absent(), Optional.<CtClass>of(newCtClass), JApiChangeStatus.NEW, classType);
                if (includeClass(jApiClass)) {
                    classes.add(jApiClass);
                }
            }
